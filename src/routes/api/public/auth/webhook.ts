 ...
 * dashboard under Authentication > Hooks: type HTTPS, URL
 * https://launchplanner.com.au/api/public/auth/webhook, secret in
 * SEND_EMAIL_HOOK_SECRET (format "v1,whsec_...", generated by that same
 * dashboard page).
 *
 * This used to live at /lovable/email/auth/webhook and run through a
 * Lovable-hosted relay (@lovable.dev/webhooks-js verified the signature;
 * @lovable.dev/email-js parsed the payload). The Lovable project has since
 * been deleted, so nothing ever called that route - Supabase silently fell
 * back to its own unbranded emails. This verifies and parses Supabase's own
 * native Send Email Hook payload directly instead, independent of Lovable.
 *
...
export const Route = createFileRoute("/api/public/auth/webhook")({
  server: {
    handlers: {
      POST: async ({ request }) => {
